...
function TI({ value, onChange, ph }: { value: string; onChange: (v: string) => void; ph?: string }) {
  return <input type="text" value={value} onChange={(e) => onChange(e.target.value)} placeholder={ph} className="w-full rounded-lg border border-neutral-600 bg-neutral-800 px-3 py-1.5 text-sm text-neutral-200 outline-none focus:border-indigo-500" />
}

function TA({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  return <textarea value={value} onChange={(e) => onChange(e.target.value)} rows={2} className="w-full resize-none rounded-lg border border-neutral-600 bg-neutral-800 px-3 py-1.5 text-sm text-neutral-200 outline-none focus:border-indigo-500" />
}

function NI({ value, onChange, min, max }: { value: number; onChange: (v: number) => void; min?: number; max?: number }) {
  return <input type="number" value={value} onChange={(e) => onChange(Number(e.target.value))} min={min} max={max} step={0.1} className="w-full rounded-lg border border-neutral-600 bg-neutral-800 px-3 py-1.5 text-sm text-neutral-200 outline-none focus:border-indigo-500" />
}

function SL({ value, onChange, opts }: { value: string; onChange: (v: string) => void; opts: { v: string; l: string }[] }) {
  return <select value={value} onChange={(e) => onChange(e.target.value)} className="w-full rounded-lg border border-neutral-600 bg-neutral-800 px-3 py-1.5 text-sm text-neutral-200 outline-none focus:border-indigo-500">{opts.map((o) => <option key={o.v} value={o.v}>{o.l}</option>)}</select>
}

function CB({ value, onChange, label }: { value: boolean; onChange: (v: boolean) => void; label: string }) {
  return <label className="flex items-center gap-2"><input type="checkbox" checked={value} onChange={(e) => onChange(e.target.checked)} className="h-4 w-4 rounded border-neutral-600 bg-neutral-800 text-indigo-500" /><span className="text-sm text-neutral-300">{label}</span></label>
}

export default function ComponentFields({ comp, onUpdate }: { comp: SceneComponent; onUpdate: (p: Record<string, unknown>) => void }) {
  const p = comp.props
  switch (comp.type) {
    case 'background': return (<><F label="Archivo"><TI value={p.file as string} onChange={(v) => onUpdate({ file: v })} ph="imagen.png" /></F><F label="Transicion"><SL value={p.transition as string} onChange={(v) => onUpdate({ transition: v })} opts={[{ v: 'none', l: 'Ninguna' }, { v: 'fade', l: 'Fade' }, { v: 'dissolve', l: 'Disolver' }]} /></F><F label="Duracion (ms)"><NI value={p.duration as number} onChange={(v) => onUpdate({ duration: v })} min={0} /></F></>)
    case 'dialog': return (<><F label="Personaje"><TI value={p.character as string} onChange={(v) => onUpdate({ character: v })} /></F><F label="Texto"><TA value={p.text as string} onChange={(v) => onUpdate({ text: v })} /></F><F label="Expresion"><TI value={p.expression as string} onChange={(v) => onUpdate({ expression: v })} /></F><F label="Posicion"><SL value={p.position as string} onChange={(v) => onUpdate({ position: v })} opts={[{ v: 'left', l: 'Izquierda' }, { v: 'center', l: 'Centro' }, { v: 'right', l: 'Derecha' }]} /></F></>)
    case 'character': return (<><F label="Personaje"><TI value={p.character as string} onChange={(v) => onUpdate({ character: v })} /></F><F label="Accion"><SL value={p.action as string} onChange={(v) => onUpdate({ action: v })} opts={[{ v: 'show', l: 'Mostrar' }, { v: 'hide', l: 'Ocultar' }, { v: 'move', l: 'Mover' }]} /></F><F label="Expresion"><TI value={p.expression as string} onChange={(v) => onUpdate({ expression: v })} /></F><F label="Posicion"><SL value={p.position as string} onChange={(v) => onUpdate({ position: v })} opts={[{ v: 'left', l: 'Izquierda' }, { v: 'center', l: 'Centro' }, { v: 'right', l: 'Derecha' }]} /></F></>)
    ...
    case 'audio': return (<><F label="Archivo"><TI value={p.file as string} onChange={(v) => onUpdate({ file: v })} /></F><F label="Tipo"><SL value={p.audioType as string} onChange={(v) => onUpdate({ audioType: v })} opts={[{ v: 'music', l: 'Musica' }, { v: 'sfx', l: 'Efecto' }]} /></F><F label="Volumen"><NI value={p.volume as number} onChange={(v) => onUpdate({ volume: v })} min={0} max={1} /></F><CB value={p.loop as boolean} onChange={(v) => onUpdate({ loop: v })} label="Repetir" /></>)
